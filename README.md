...
│   ├── test_cli.py                    # CLI (validate-ncf, run) tests
│   ├── test_email_ingestor.py         # IMAP ingestion tests (mocked)
...
│   ├── test_ocr.py
│   ├── test_pipeline.py
│   ├── test_tesseract_client.py       # Local OCR fallback tests
...
- [ ] Add `mypy` as an enforced step in the CI workflow (it is currently
      configured in `pyproject.toml` and installable via the `dev` extra,
      but only run locally / on demand, not gated in `ci.yml`).
