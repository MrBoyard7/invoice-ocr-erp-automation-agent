...
### Fixed
- `README.md`'s project structure tree was missing `test_cli.py`,
  `test_email_ingestor.py`, and `test_tesseract_client.py`, added in an
  earlier revision but never reflected in the tree.
- `erp/models.py` used `Mapped[float]` for monetary columns
  (`subtotal`, `tax_amount`, `total_amount`) while `extraction/schemas.py`
  uses `Decimal` for the same values, and SQLAlchemy's `Numeric` type
  already returns `Decimal` by default. Changed to `Mapped[Decimal]` for
  correctness (avoids floating-point rounding on monetary values) and type
  consistency between the domain model and the ERP model.
- `erp/repository.py`: added an explicit
  `entry: AccountsReceivableEntry | AccountsPayableEntry` annotation so the
  variable's type is tracked correctly across the `if`/`else` branches
  (this is a local-variable annotation, never evaluated at runtime, so it
  remains safe on Python 3.9 despite the `|` syntax).
- Documented in the Roadmap that `mypy` is configured but not yet an
  enforced step in `ci.yml`, so the "type-checked: mypy" badge isn't
  overstating what CI currently verifies.
